...
module.exports = function (app) {
    ...
    app.get(`/${endpoint_category}/get_accounts`, async (request, response) => {
        ...
        */
        ...
        if (request.query.token) {
            ...
            let accountsExists = await Accounts.find({ token: request.query.token})
            ...
            if (!functions.empty(userExists)) {
                try {
                    ...
                    payload["accounts_lenght"] = accountsExists.accounts.length
                    cache.set(cache_key, accountsExists);
                    response.status(200).json({ "status": 200, "message": "Accounts has been fetched successfully.", "data": payload });
