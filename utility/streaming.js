...
broadcast_youtube: ( title, description, scheduledStartTime, refreshtoken, userToken, videoId, callback ) => {
        ...
        request_url.post(url, {form: body, json: true}, async (err, res, body) => {
            ...
            await Accounts.updateOne(
                { "token": userToken, "accounts.name": "youtube" },
                { "$set": { 
                        "accounts.$.access_token": body.access_token,
                        "accounts.$.refresh_token": body.refresh_token,
...
get_twitch_stream_key: ( user_refresh_token, twitch_id, userToken, title, callback ) => {
    ...
    request_url.post(url, {form: body, json: true}, async (err, res, body) => {
        if(err){
            ...
            await Accounts.updateOne(
                { "token": userToken, "accounts.name": "twitch" },
                { "$set": { 
                        "accounts.$.access_token": body.access_token,
                        "accounts.$.refresh_token": body.refresh_token,
                        "accounts.$.is_connected": true
